...
(function() {
    ...
    function procesarRespuesta(texto) {
        ...
        const quiereVerProyectos = lowerText.includes('terren') || lowerText.includes('lote') || lowerText.includes('casa') || lowerText.includes('proyect');
        
        if ((estado.paso === 'cotizacion_inicial' || estado.paso === 'cotizacion_plazo' || estado.paso === 'cotizacion_proyecto') && quiereVerProyectos) {
            // Resetear el paso y mostrar proyectos
            estado.paso = 'inicio';
            mostrarProyectos();
            return;
        }
        
        // Si está cotizando y menciona un proyecto diferente al activo, cambiar de proyecto
        const proyectoActivo = estado.datosCotizacion.proyecto;
        const mencionaPalmaReal = contieneAlguna(lowerText, ['palma real', 'pimentel', 'playa']);
        ...
        // Detectar saludos pero NO retornar inmediatamente - continuar buscando intenciones
        const tieneSaludo = contieneAlguna(lowerText, KNOWLEDGE_BASE.saludos) || lowerText === 'hola';
        
        // Si tiene saludo Y también menciona proyectos/ubicación específica, procesar la consulta específica
        const mencionaProyecto = contieneAlguna(lowerText, ['palma real', 'pimentel', 'reque', 'requemar']);
        const mencionaProyectosGeneral = contieneAlguna(lowerText, ['proyectos', 'lotes', 'terrenos', 'casas']);
        
        if (tieneSaludo && !mencionaProyecto && !mencionaProyectosGeneral && !contieneAlguna(lowerText, KNOWLEDGE_BASE.cotizacion)) {
            // Solo saludo sin consulta específica - mostrar bienvenida
            estado.paso = 'inicio';
            cambiarImagen('bienvenida');
            mostrarMensajeBienvenida();
            return;
        ...
        // Detectar consultas sobre proyectos/terrenos disponibles
        if (lowerText.includes('terren') || lowerText.includes('lote') || lowerText.includes('casa') || lowerText.includes('proyect')) {
            mostrarProyectos();
            return;
        }
        // Detectar preguntas sobre disponibilidad
        if ((lowerText.includes('que') || lowerText.includes('cuales') || lowerText.includes('tienes') || lowerText.includes('hay')) && 
            (lowerText.includes('tienes') || lowerText.includes('disponible') || lowerText.includes('hay') || lowerText.includes('opciones'))) {
            mostrarProyectos();
            return;
        }

        // Detectar solicitud de cotización/cuotas
        const quiereCotizar = contieneAlguna(lowerText, KNOWLEDGE_BASE.cotizacion) || lowerText.includes('cotizar') || lowerText.includes('cuotas') || lowerText.includes('pagar al mes') || lowerText.includes('en cuantas cuotas');
        
        if (quiereCotizar) {
            // Verificar si hay un proyecto activo guardado en el contexto
            const proyectoActivo = estado.contextoProyecto || estado.datosCotizacion.proyecto;
            
            // Si hay proyecto activo y el mensaje NO menciona un proyecto específico diferente
            const mencionaPalmaReal = contieneAlguna(lowerText, ['palma real', 'pimentel']);
            const mencionaReque = contieneAlguna(lowerText, ['reque', 'requemar']);
            
            if (proyectoActivo && !mencionaPalmaReal && !mencionaReque) {
                // Usar el proyecto activo del contexto
                estado.paso = 'cotizacion_inicial';
                estado.datosCotizacion.proyecto = proyectoActivo;
                const precio = proyectoActivo === 'palma real' ? 85000 : 72000;
                estado.datosCotizacion.precio = precio;
                
                const nombreProyecto = proyectoActivo === 'palma real' ? 'Palma Real' : 'Reque Mar';
                cambiarImagen('explicando');
                agregarMensaje(
                    '¡Perfecto! Como estamos hablando de <strong>' + nombreProyecto + '</strong>, te ayudo con las cuotas 💰<br><br>' +
                    'Precio: ' + precio.toLocaleString() + ' soles<br><br>' +
                    '¿Qué porcentaje puedes pagar al inicio?<br>' +
            ...
            // No hay proyecto activo o el usuario mencionó uno específico
            // Determinar qué proyecto mencionó
            // Si menciona un proyecto específico, mostrar info primero
            if (mencionaPalmaReal) {
                mostrarInfoProyecto('palma real');
            } else if (mencionaReque) {
    ...
    function iniciarCotizacion(proyectoEspecifico) {
        // Si se especifica un proyecto, ir directamente a la pregunta de cuota inicial
        if (proyectoEspecifico && CONFIG.proyectos[proyectoEspecifico]) {
            estado.paso = 'cotizacion_inicial';
            ...
            cambiarImagen('explicando');
            agregarMensaje(
                '¡Perfecto! Vamos a cotizar <strong>' + nombreProyecto + '</strong> 💰<br><br>' +
                'Precio: ' + precio.toLocaleString() + ' soles<br><br>' +
                '¿Qué porcentaje puedes pagar al inicio?<br>' +
                '(Mínimo 5%, máximo 50%)<br><br>' +
                'Por ejemplo: "10" o "20"',
        ...
        // Sin proyecto específico - preguntar cuál terreno
